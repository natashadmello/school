...
public:
    ...
    // Estimate of performance: O(n)
    // Short rationale for estimate: Iterating through the connections has a linear time complexity.
    std::vector<Connection> get_connected_affiliations(AffiliationID id);

    // Estimate of performance: O(n^(2)logn)
    // Short rationale for estimate: O(n^2) complexity to check uniqueness, additional complexity to check sorting.
    std::vector<Connection> get_all_connections();

    // Estimate of performance: O(n^2)
    // Short rationale for estimate: Linearly dependent on the size of the graph during DFS traversal.
    Path get_any_path(AffiliationID source, AffiliationID target);

    // PRG2 optional functions

    // Estimate of performance: O(nlogn)
    // Short rationale for estimate: Priority queue is used to find the path with the fewest affiliations.
    Path get_path_with_least_affiliations(AffiliationID source, AffiliationID target);

    // Estimate of performance: O(n)
    // Short rationale for estimate: Linearly proportional to affiliations and connections.
    Path get_path_of_least_friction(AffiliationID source, AffiliationID target);


    // Estimate of performance: O(nlogn)
    // Short rationale for estimate: Shortest path is calculated using Euclidean distances.
...
private:
    ...
    //A struct that is used for storing the necessary affiliation informations
        ...
        // Container to store Connection objects
